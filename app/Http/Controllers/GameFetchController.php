...
//            ->where('end_date', '>', $now)
            ...
            ->map(function ($query) {
            $formattedStartDate = (new DateTime($query->start_date))->format('d.m.y H:i');
            $formattedEndDate = (new DateTime($query->end_date))->format('d.m.y H:i');
            
            $sdo = new DateTime($query->start_date);
            $cd = new DateTime();
            $td = $sdo->diff($cd);
             $totalMinutesDifference = $td->days * 24 * 60 + $td->h * 60 + $td->i;

                return [
                    "id" => $query->id,
                    'startminutes' => $totalMinutesDifference,
        ...
        $past = $matches->where('end_date', '<', $now->addHours(66))->values();
